...
def main():
    ...
    # only better
    listdir = listdir[:all_zeros_idx + 1]
    plt_data = plt_data[:all_zeros_idx + 1]

    index_min = min(range(len(plt_data)), key=plt_data.__getitem__)
    plt.plot(plt_data, listdir, 'k.',
             label=f'Average {selected_field} by {results_path} from ACP runs with different coefficients')
